            ...
            if currLocation then
                ...
                if #(GetEntityCoords(PlayerPedId()) - Config.Locations[currLocation].control) <= 1.5 then
                    if not isBusy[currLocation] then
                        if not isDrawn then
                            _ShowHelpNotification('Aperte [E] para abrir as opções de pintura')
                            isDrawn = true
                        end
                    ...
                    elseif not isDrawn then
                        _ShowHelpNotification('A sala de pintura está em uso')
                        isDrawn = true
                    end
                elseif isDrawn then
                    lib.hideTextUI()
...
InitializePaint = function(pos)
    ...
    if not vehicle then return _ShowNotification('O veiculo não esta na posição') end
    ...
    local input = lib.inputDialog('Pintura de veículos', {
        ...
        { type = 'select', label = 'Tipo', options = {
            ...
            { value = '2', label = 'perolado' },
            { value = '3', label = 'Fosco' },
            { value = '4', label = 'Metalico' },
            { value = '5', label = 'Cromado' },
